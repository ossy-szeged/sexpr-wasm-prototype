...
FUZZ_CC := ${FUZZ_BIN_DIR}/afl-gcc
FUZZ_CXX := ${FUZZ_BIN_DIR}/afl-g++
...
FUZZ_FLAG := -DCMAKE_C_COMPILER=${FUZZ_CC} -DCMAKE_CXX_COMPILER=${FUZZ_CXX}
...
#
# One-off build target for fuzzing
$(eval $(call CMAKE,FUZZ,DEBUG,-DBUILD_TESTS=OFF))
$(eval $(call EXE,FUZZ,DEBUG,NO))
